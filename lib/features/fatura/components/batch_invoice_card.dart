...
import 'firma_secici_dialog.dart';
import 'hizmet_secici_dialog.dart';
import '../screens/visual_entry_screen.dart';

class BatchInvoiceCard extends StatefulWidget {
  final FaturaModel invoice;
  final int index;
  final bool isExpanded;
  final VoidCallback onToggle;

  const BatchInvoiceCard({
    super.key,
  ...
  @override
  State<BatchInvoiceCard> createState() => _BatchInvoiceCardState();
}

class _BatchInvoiceCardState extends State<BatchInvoiceCard> {
  @override
  Widget build(BuildContext context) {
    ...
    final ibanEslesiyor =
        ibanKontrolYapilabilir && provider.ibanEslesiyorMu(index);
  ...
  Widget _buildFormBody(
    ...
    return Column(
      ...
      children: [
        ...
        // --- DİNAMİK ALANLAR ---
        _buildDinamikAlanlar(
          context,
          provider,
          invoice,
          index,
          provider.getBirimKategori(provider.seciliBirimFor(index)),
        ),
            ...
            children: [
              Expanded(
                child: _field(
                  ...
                  cardIndex: index,
                  rebuildKey: ValueKey(
                    'ekstra_not_${index}_${i}_$dialogCounter',
                  ),
                ),
              ),
              IconButton(
                  ...
                  onPressed: () {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Yeni kalem listenin en altına eklendi.',
                          ),
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: _KalemFiyatAlani(
                        ...
                        value: provider.parseTurkceSayi(
                          k['fiyat'],
                          fallback: 0,
                        ),
                        onChanged: (v) =>
                            provider.updateKalem(index, ki, 'fiyat', v),
                      ),
                    ),
                    const SizedBox(width: 6),
                    Container(
            ...
            onPressed: () async {
              final onay = await showDialog<bool>(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: const Text(
                    'Bu faturayı listeden silmek istediğinize emin misiniz?',
                  ),
                  actions: [
                    TextButton(
            ...
            onPressed: () async {
              final onay = await showDialog<bool>(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: const Text(
                    'Bu faturanın birebir kopyası oluşturulup listeye eklensin mi?',
                  ),
                  actions: [
                    TextButton(
                      onPressed: () => Navigator.pop(ctx, false),
            ...
            children: [
              ...
              Expanded(
                child: CheckboxListTile(
                  ...
                  onChanged: (v) =>
                      provider.updateField(index, 'ytbOgrencisi', v),
